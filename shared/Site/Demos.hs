...
-----------------------------------------------------------------------------
-- >>> context
-- This site's context holds the language and the theme. Any component can
-- read it (first argument of view) and change it (modifyContext). Only
-- components with useContext = True re-render when it changes.
data ThemeAction = FlipTheme

themeSwitch :: Component Ctx () () ThemeAction
themeSwitch = (component () update view)
  { useContext = True }
  where
    update FlipTheme = do
      ctx <- getContext                       -- context is readable in Effect
      let theme = if ctxTheme ctx == Dark then Light else Dark
      modifyContext (\c -> c { ctxTheme = theme })
      io_ $ do                                -- persist + apply, like the top bar
    ...
    view ctx () () =
      H.div_ []
        [ H.p_ []
            [ "The context says: theme = ", H.strong_ [] [ text (ms (show (ctxTheme ctx))) ]
            , ", language = ", H.strong_ [] [ text (langName (ctxLang ctx)) ]
            ]
        , H.button_ [ HE.onClick FlipTheme ] [ "Flip the whole site's theme" ]
        , "frozen" +> frozen
        ]

-- A sibling that does not opt in: it keeps showing the context it mounted with.
frozen :: Component Ctx () () ()
frozen = component () (\() -> pure ()) view    -- useContext defaults to False
  where
    view ctx () () =
...
-----------------------------------------------------------------------------
-- >>> json
data User = User { userName :: MisoString, userAge :: Int }
...
orbits = (component 0 update view) { subs = [ rAFSub Frame ] }
  where
